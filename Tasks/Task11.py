...
def print_info(obj: General):
    print(obj.print_object())


def safe_call(obj: General):
    if isinstance(obj, Void):
        print("Нет значения (Void)!")
    else:
        print("Объект:", obj.print_object())


if __name__ == "__main__":
    p = Person("Анна", 20)
    c = Car("Ford", "Focus", 2018)

    print_info(p)
    print_info(c)
    print_info(VoidInstance)

    safe_call(p)
    safe_call(VoidInstance)
